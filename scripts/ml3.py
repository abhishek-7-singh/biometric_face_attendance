...
def mark_final_attendance(name, present):
    ...
    append_csv(r"C:\Users\abhi1\Desktop\BIOMETRIC\scripts\dataa\final_attendance2.csv", [name, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), status])

def append_csv(path, row):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    write_header = not os.path.exists(path)
    
    # Prevent duplicate entries with same name and status
    if "final_attendance.csv" in path:
...
entry_thread = threading.Thread(target=process_camera, args=(0, "Entry"))  # Built-in laptop camera
exit_thread = threading.Thread(target=process_camera, args=(1, "Exit"))   # DroidCam virtual USB cam (likely 1 or 2)
...
# exit_thread = threading.Thread(target=process_camera, args=("http://192.168.167.92:8080/video", "Exit"))  # Exit camera
