...
entry_logs = {}
break_logs = {}
class_duration = timedelta(minutes=1)
final_marked = set()
...
def log_return_from_break(name):
    now = datetime.now()
    if name in break_logs and (now - break_logs[name]).seconds <= 3:  # 5 minutes
...
exit_thread = threading.Thread(target=process_camera, args=("http://192.168.137.35:8080/video", "Exit"))  # Exit camera
